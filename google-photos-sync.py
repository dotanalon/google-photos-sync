...
class GooglePhotosSync(object):
	...
	self.gd_client = gdata.photos.service.PhotosService(self.gauth)
...
def upload_command(googlePhotosSync, args):
    ...
    for local_media in googlePhotosSync.get_local_medias():
        ...
        if args.dry_run:
            print "uploading %s" % local_media.filename
            continue
...
def list_albums_command(googlePhotosSync, args):
    print("....")    
...
parser.add_argument(
    "command",
    metavar="COMMAND",
    choices=["upload", "re-upload", "download", "list-albums"],
...
if args.command == "download":
    ...
    upload_command(googlePhotosSync, args)

elif args.command == "upload":
    list_albums_command(googlePhotosSync, args)
